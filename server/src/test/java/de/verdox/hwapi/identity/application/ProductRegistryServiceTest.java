...
import de.verdox.hwapi.identity.ProductIdentifier.IdentifierType;
...
class ProductRegistryServiceTest {
    ...
    @Test
    void registerUsesTheSameCanonicalTitleValueForLookupAndInsert() {
        when(productIdentifierRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        ProductIdentifier registered = service.register(TITLE_NORMALIZED, "Ryzen 7 2700X (boxed)", "AMD");

        verify(productIdentifierRepository).findByTypeAndIdentifier(TITLE_NORMALIZED, "ryzen 7 2700x");
        assertThat(registered.getValue()).isEqualTo("Ryzen 7 2700X (boxed)");
        assertThat(registered.getNormalizedValue()).isEqualTo("ryzen 7 2700x");
    }

    @Test
    void registerReusesLegacyCaseVariantInsteadOfInsertingItAgain() {
        ProductIdentifier legacy = identifier(99L, TITLE_NORMALIZED, "Ryzen 7 2700X");
        when(productIdentifierRepository.findFirstByTypeAndNormalizedValueIgnoreCase(
                TITLE_NORMALIZED, "ryzen 7 2700x")).thenReturn(Optional.of(legacy));
        when(productIdentifierRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        ProductIdentifier registered = service.register(TITLE_NORMALIZED, "Ryzen 7 2700X", "AMD");

        assertThat(registered).isSameAs(legacy);
        verify(productIdentifierRepository, times(1)).save(legacy);
    }
