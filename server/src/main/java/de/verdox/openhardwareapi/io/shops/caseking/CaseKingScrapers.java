    ...
    public static AbstractMultiCasekingScraper<CPU> forCPU(HardwareSpecService service) {
        return new AbstractMultiCasekingScraper.Builder<CPU>()
                ...
                .withSpecsTranslation((cpu, specs) -> {
                    cpu.setManufacturer(extractFirstString("CPU-Hersteller", specs));
                    cpu.setSocket(extractFirstEnum(HardwareTypes.CpuSocket.class, "CPU-Sockel", specs, (s, cpuSocket) -> s.contains(cpuSocket.name())));
                    cpu.setCores((int) parseFirstInt("CPU-Kerne", specs));
                    cpu.setThreads((int) parseFirstInt("CPU-Threads", specs));
                    cpu.setL3CacheMb((int) parseFirstInt("L3-Cache", specs));
                    cpu.setTdpWatts((int) parseFirstInt("TDP", specs));
                    cpu.setBaseClockMhz(parseFirstDouble("max. CPU-Takt (Basis)", specs) * 1000);
                    cpu.setBoostClockMhz(parseFirstDouble("max. CPU-Takt (Turbo / Boost)", specs) * 1000);
                })
                .build();
    }
