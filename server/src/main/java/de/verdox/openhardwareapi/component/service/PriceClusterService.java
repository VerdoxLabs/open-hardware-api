...
// ---------- Helper für AVG ----------
    ...
    private Price getAveragePricePerGB(List<RAM> ramList, Currency currency, int monthSince) {
        ...
        Map<String, Integer> eanToGb = ramList.stream()
                ...
                .collect(Collectors.toMap(RAM::getMPN, RAM::getTotalSizeGB, (a, b) -> a));
    ...
    public Price estimateRamStickPrice(
        ...
        // Minimal-Floor für DDR4-DIMM Single-Stick
        BigDecimal floor = new BigDecimal("14.90");

        BigDecimal result = withAdj.max(floor).setScale(2, RoundingMode.HALF_UP);
