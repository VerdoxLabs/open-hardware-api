...
public abstract class HardwareSpec<SELF extends HardwareSpec<SELF>> {
    ...
    public void addEAN(String ean) {
        ...
        if (normalized != null) this.EANs.add(normalized);
    }

    public void addMPN(String mpn) {
        String normalized = normalizeMpn(mpn);
        if (normalized != null) this.MPNs.add(normalized);
    }

    // Mini-Helper
    public static String normalizeEan(String raw) {
        return IdentifierNormalizer.gtin(raw);
    }
    ...
    public void setEANs(Set<String> eans) {
        // Keep Hibernate's PersistentSet instance when this entity is managed.
        // Replacing it with HashSet breaks collection hydration/dirty tracking.
        this.EANs.clear();
        if (eans != null) eans.forEach(this::addEAN);
    }

    public void setMPNs(Set<String> mpns) {
        this.MPNs.clear();
        if (mpns != null) mpns.forEach(this::addMPN);
    }
    ...
    public void sanitizeNumbers() {
        ...
        // Do not replace persistent collections in a JPA lifecycle callback.
        this.EANs.clear();
        this.MPNs.clear();
