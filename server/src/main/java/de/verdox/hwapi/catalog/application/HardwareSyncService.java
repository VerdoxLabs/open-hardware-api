...
import de.verdox.hwapi.integration.client.HardwareSpecClient;
...
public class HardwareSyncService {
    ...
    private void triggerFlushAsync() {
        ...
        ConcurrentMap<String, HardwareSpec<?>> drained = activeBuffer.getAndSet(new ConcurrentHashMap<>());
        ...
        syncExecutor.submit(() -> {
            try {
                ...
                LOGGER.log(Level.SEVERE, "An error occurred while flushing the sync queue.", e);
            }
        });
    }
    ...
    private void flushBuffer(ConcurrentMap<String, HardwareSpec<?>> batch) {
        ...
        if (total > 0) {
            LOGGER.log(Level.INFO, "Flushing {0} specs (bulk) to {1} clients ...", new Object[]{total, clients.size()});
        }
        ...
        for (HardwareSpecClient client : clients) {
            ...
            for (Map.Entry<String, List<HardwareSpec<?>>> e : byType.entrySet()) {
                ...
                for (List<HardwareSpec<?>> chunk : partition(specsOfType, bulkMaxBatchSize)) {
                    for (HardwareSpec<?> hardwareSpec : chunk) {
                        if (hardwareSpec.getMPNs().isEmpty() && hardwareSpec.getEANs().isEmpty()) {
                            skippedCounter++;
                            continue;
                        }
            ...
            if (counter > 0) {
                LOGGER.log(Level.INFO, "\tSynced " + counter + " hardware entities to " + client.getUrlApiV1());
            }
            if (skippedCounter > 0) {
                LOGGER.log(Level.WARNING, "\tCould not sync " + skippedCounter + client.getUrlApiV1());
            }
        }
    ...
    private String buildKey(String type, HardwareSpec<?> spec) {
        String id = firstNonBlank(spec.getMPNs().stream().sorted().toList().toString(),
    ...
    private String safeKeyOf(HardwareSpec<?> spec) {
        return firstNonBlank(spec.getMPNs().stream().sorted().toList().toString(), "?");
    }
